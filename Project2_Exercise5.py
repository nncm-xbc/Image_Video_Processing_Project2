...
shiftA = np.fft.fftshift(dftA)
shiftB = np.fft.fftshift(dftB)
...
plt.subplot(121), plt.imshow(np.log(1+np.abs(shiftA)), cmap='gray')
plt.title('Spectral appearance of A'), plt.xticks([]), plt.yticks([])
plt.subplot(122), plt.imshow(np.log(1+np.abs(shiftB)), cmap='gray')
